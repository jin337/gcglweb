...
export default {
  ...
  methods: {
    ...
    isRowSelected(row) {
      return this.allSelectData.some(item => 
        item.device_id === row.device_id && item.device_model === row.device_model && item.device_brand === row.device_brand
      );
    },
    
    tableRowClassName({ row }) {
      return this.isRowSelected(row) ? 'selected_dev' : '';
    },
    addDevice(row) {
