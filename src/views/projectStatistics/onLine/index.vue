    ...
    <div class="_right" v-loading="rightLoading">
      ...
      <el-form class="header" inline size="mini">
        ...
        <el-form-item label="状态">
          <el-select
            v-model="rightForm.check_status"
            placeholder="请选择"
            style="width: 100px"
          >
            <el-option
...
export default {
  ...
  methods: {
      ...
      if (this.range && this.range[0] && this.range[1]) {
        this.leftForm.check_date_begin = moment(this.range[0]).format('YYYY-MM-DD')
        this.leftForm.check_date_end = moment(this.range[1]).format('YYYY-MM-DD')
      } else {
        this.leftForm.check_date_begin=""
        this.leftForm.check_date_end=""
      }
    },
    getTableHeight() {
      this.$nextTick(() => {
        ...
        if (el_wrap) {
          ...
          this.leftTableHeight =
            _leftel.offsetHeight -
            (title1.offsetHeight + header1.offsetHeight + 30);
          this.rightTableHeight =
            _rightel.offsetHeight -
            (title2.offsetHeight + header2.offsetHeight + 30);
        }
      });
    },
    async getProject() {
    ...
    async getLeftData() {
      if (!this.leftForm.project_code || (!this.leftForm.check_date_begin&& !this.leftForm.check_date_end)) {
        return;
      }
      this.leftLoading = true;
      const params = {
        project_code: this.leftForm.project_code,
        check_date_begin: this.leftForm.check_date_begin,
        check_date_end: this.leftForm.check_date_end
      };
      const { data, code } = await this.$pub.post(
        "rate/project_online_device_sum",
        params
      );
      if (code !== 200) {
        return this.$message({
      ...
      this.leftData = (data || []).map((m) => {
        return {
          ...m,
          percentage: (m.ok_status / m.sum).toFixed(2) * 100 + "%",
        };
      });
    ...
    async getRightData() {
      ...
      const { data, code } = await this.$pub.post(
        "rate/project_online_device_detail",
        params
      );
      if (code !== 200) {
        return this.$message({
      ...
      this.rightData = (data || []).map((m) => {
        return {
          ...m,
          check_status_name: m.check_status === "1" ? "在线" : "不在线",
        };
      });
