<template>
  <div class="projectManagement setting_wrap">
    <div class="searchwrap">
      ...
      <el-date-picker style="width:220px;" :picker-options="pickerOptions" v-model="form.finish_date" align="right"
        placeholder="请选择完成时间" value-format="yyyy-MM-dd">
      </el-date-picker>
      <div class="btns">
        ...
        <template slot-scope="scope">
          <el-dropdown @command="handleCommand" style="margin-right:10px;" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item :command="beforeHandleCommand(scope.row, 'authorize')">授权</el-dropdown-item>
              ...
              <el-dropdown-item v-hasPermi="['projectManagement:proc']"
                :command="beforeHandleCommand(scope.row, 'procM')">工序物料维护</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <add-edit ref="addedit" v-if="open" :open.sync="open" :current="currentRow" :statusList="statusList"
        @handleQuery="handleQuery"></add-edit>
    </el-dialog>

    <!-- 授权 -->
    <el-dialog :visible.sync="authorizeFlag" :footer="false" width="800px" title="授权">
      <authorize-setting :current="currentRow" v-if="authorizeFlag" :authorizeFlag.sync="authorizeFlag"
        @handleQuery="handleQuery"></authorize-setting>
    </el-dialog>

    <!-- 子系统维护 -->
    <el-dialog :visible.sync="childFlag" :footer="false" width="800px" title="子系统维护">
      <child-setting :current="currentRow" v-if="childFlag"></child-setting>
    </el-dialog>

    <!-- 工程阶段维护 -->
    <el-drawer title="工程阶段维护" :visible.sync="stageFlag" :append-to-body="true" custom-class="stage_wrap" size="75%"
      :destroy-on-close="true">
      <stage-setting :stageFlag.sync="stageFlag" :project_code="currentRow.project_code" :project_id="currentRow.id"
        v-if="stageFlag"></stage-setting>
    </el-drawer>

    <!-- 厂商平台维护 -->
    <el-drawer title="厂商平台维护" :visible.sync="companyFlag" :append-to-body="true" custom-class="company_wrap" size="80%"
      :destroy-on-close="true">
      <company-setting :companyFlag.sync="companyFlag" :project_code="currentRow.project_code"
        :project_id="currentRow.id" v-if="companyFlag"></company-setting>
    </el-drawer>

    <!-- 工序物料维护 -->
    <el-drawer title="工序物料维护" :visible.sync="procMFlag" :append-to-body="true" custom-class="procm_wrap" size="90%"
      :destroy-on-close="true">
      <procm-setting :procMFlag.sync="procMFlag" :project_code="currentRow.project_code" :project_id="currentRow.id"
        v-if="procMFlag"></procm-setting>
    </el-drawer>

    <!-- 项目排序-->
    <el-drawer title="项目排序" :visible.sync="sortFlag" :append-to-body="true" custom-class="sort_wrap" size="700px"
      :destroy-on-close="true" @closed="handleQuery">
...
import authorizeSetting from './authorizeSetting.vue'
...
export default {
  ...
  components: {
    addEdit, childSetting, stageSetting, companySetting, procmSetting, sortProject, authorizeSetting
  },
  methods: {
    ...
    handleCommand (command) {
      ...
      } else if (command.command === 'authorize') {
        this.handleAuthorizeSetting()
      }
    },
    showSort () {
      this.sortFlag = true
