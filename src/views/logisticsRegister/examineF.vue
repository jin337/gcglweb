<template>
  <div class="logExamineF clover_logreg_container" v-loading="loading">
    <!-- 查询 -->
        ...
        <div style="font-size:18px;" v-if="!isLock(currentPoint)">
          <el-button-group>
            ...
            <el-tooltip class="item" effect="dark" content="待审工序" placement="top-start">
              <el-button size="small" @click="changeIsList('1')" :style="{ color: isStatus == '1' ? '#409EFF' : '' }">
                <img
            ...
            <el-tooltip class="item" effect="dark" content="审核未通过工序" placement="top-start">
              <el-button size="small" @click="changeIsList('2')" :style="{ color: isStatus === '2' ? '#409EFF' : '' }">
                <img
      ...
      <div class="wrap scrollWrap" v-if="procLists.length > 0">
        <el-collapse :value="activeClassName">
          <el-collapse-item v-for="item in procLists" :key="item.class_Name" :title="item.class_name"
            ...
            <div v-if="item.device_attach_list.length > 0">
              ...
              <div class="proc-item" v-for=" (deviceimg, k) in item.device_attach_list" :key="deviceimg.device_id + k">
                <div class="deviceimg_title">
                  ...
                  <el-button type="primary" size="mini" @click="handleDeviceImgCheck(deviceimg)"
                    v-if="deviceimg.haveCheck && !isLock(currentPoint)">审核</el-button>
                </div>
                  ...
                  <div class="box devimgbox" v-for="proc in deviceimg.device_att_proc" :key="proc.proc_code">
                    ...
                    <div class="box_img">
                      ...
                      <el-tooltip effect="dark" placement="right-end" v-if="proc.remark || proc.lastTime">
                        ...
                        <img :src="img.filePathThumb" style="width: 140px;" :key="proc.filePath" v-if="proc.is360 === 1"
                          @click="initPhotoSphere(proc.procName, proc.filePathTrans)" />
                      </el-tooltip>
                      <!-- 无备注信息的 照片or全场景照片-->
                      ...
                      <template v-else>
                        <img :src="proc.filePathThumb" :data-source="proc.filePathTrans" :key="proc.filePath"
                          style="width: 140px;" v-if="proc.is360 !== 1" />
                        <img :src="img.filePathThumb" style="width: 140px;height:140px;" :key="proc.filePath"
                          v-if="proc.is360 === 1" @click="initPhotoSphere(proc.procName, proc.filePathTrans)" />
                      </template>
                      <div class="sett" @click="materDeviceSetting(proc)"
                        v-if="!isLock(currentPoint) && checkPermission(['logisticsRegister:proc']) && proc.is_fill_device === 1">
                        <i :class="{ 'el-icon-setting': true, 'ishavepeizhi': proc.is_submit_count !== '' }"></i>
                      </div>
                    </div>
                    ...
                    <div
                      v-if="!isLock(currentPoint) && proc.filePath && imgExamine_f.includes(proc.audit) && second && deviceimg.haveCheck">
                      <el-radio-group v-model="proc.deviceimgRadio" class="imgRadio">
            ...
            <!-- <div v-else style="color:#848484;text-align:center;">
              暂无可审物料照片
            </div> -->
            <!-- 工序步骤照片 -->
              ...
              <div v-for="(pointDirect, i) in procitem.pointList"
                ...
                <div class="clover_wrap" v-viewer="options" v-if="pointDirect.attach_list.length > 0">
                  <div class="box" v-for="img in pointDirect.attach_list" :key="img.image_id">
                    ...
                    <div class="sett" @click="materSetting(img, procitem)"
                      v-if="!isLock(currentPoint) && checkPermission(['logisticsRegister:proc']) && img.is_fill_device === 1">
                    ...
                    <div
                      v-if="!isLock(currentPoint) && ((procitem.is_core === 1 && procitem.procCode !== 'CLS000-04') || (additional.includes(form.projectCode) && procitem.procCode === 'CLS009-04'))">
                      <el-radio-group v-model="img.num" @change="changeImgAudit(img)" class="imgRadio"
                        v-if="(imgExamine_f.includes(img.audit) && second)">
    ...
    <div class="clover_right_tab">
      <ul class="right_tab">
        <li class="item" v-for="item in tabList" @click="changeShowtab(item)" :key="item.id"
        ...
        <li class="meno">
          <el-button type="primary" size="mini" icon="el-icon-s-order" @click="handleShowMemo"
            v-if="currentPoint && checkPermission(['logisticsRegister:memoList'])">备忘录</el-button>
        </li>
      </ul>

      <!-- 物料清单 -->
      <DeviceInformation ref="deviceinfo" v-show="currentRightTab === 1" :point="currentPoint"
        :projectCode="form.projectCode" :isEdit="false" :isSaveBtn="false" :isCHU="false"
        :is_lock="currentPoint && isLock(currentPoint)" @changeLoading="changeLoading">
      </DeviceInformation>
      <!-- 链路信息 -->
      <LinkInformation v-show="currentRightTab === 2" :point="currentPoint" :projectCode="form.projectCode"
        :isEdit="false" @changeLoading="changeLoading"></LinkInformation>

      <!-- 取电信息 -->
      <PowerCessInformation v-show="currentRightTab === 3" :point="currentPoint" :projectCode="form.projectCode"
        :isEdit="false" @changeLoading="changeLoading"></PowerCessInformation>
    ...
    <el-dialog title="" :visible.sync="settingMaterFlag" :footer="false" :append-to-body="true"
      ...
      <proc-mar v-if="settingMaterFlag" :settingMaterFlag.sync="settingMaterFlag" :current="currentSettingImg"
        :project_code="form.projectCode" :point_code="currentPoint.pointCode" :isNoseting="true"></proc-mar>
    </el-dialog>

    <!-- 备忘录列表 -->
    <el-drawer title="备忘录" :visible.sync="memoModal" size="90%" :append-to-body="true" :destroy-on-close="true">
      <memo-list v-if="memoModal && currentPoint" :point_code="currentPoint.pointCode" :project_code="form.projectCode"
        :is_lock="currentPoint && isLock(currentPoint)"></memo-list>
    </el-drawer>
  </div>
</template>
...
import memoList from '../points/memo/list.vue'
export default {
  name: 'logisticsRegisterExamineF',
  data () {
    return {
      tableLoading: false,
      form: {
      ...
      timer: null,
      tableHeight: window.innerHeight - 465,
      currentPoint: null,
  ...
  components: { procMar, photo, memoList, DeviceInformation, LinkInformation, PowerCessInformation },
  methods: {
    checkPermission,
    ...
    async getPointList () {
      ...
      const { data, code, message } = await this.$pub.post('/device/build-point-list', build_param)
      if (code !== 200) {
        this.$message({
          type: 'error',
    ...
    async getStatusCount () {
      ...
      const { data, code, message } = await this.$pub.post('/device/build-point-count', build_param)
      if (code !== 200) {
        this.$message({
          type: 'error',
          ...
          (res.data || []).forEach(f => {
            ...
            this.procLists.push({
              ...
              device_attach_list: (f.device_attach_list || []).map(m => {
                /**
                 * 不展示审核按钮:
                 * 有一个photo_info为null，那整个当前物料照片不可以有审核按钮
                 * 有图片状态不是初审通过 终审通过 终审未通过，那整个当前物料照片不可以有审核按钮
                 */
                const disAgreePic = (m.device_att_proc || []).filter(device => !device.photo_info || !this.imgExamine_f.includes(device.photo_info.audit))// 不符合规定的数组
                return (this.isStatus === '1' && disAgreePic.length > 0) ? {} : { // 待审工序状态下没有审核按钮返回空
                  device_id: m.device_id,
                  device_name: m.device_nam,
                  name: m.device_name + '_' + m.no,
                  haveCheck: disAgreePic.length <= 0,
    ...
    /**
     * 物料照片审核
     * 1.同一个工序步骤下面的物料照片必须都有，有一个pointinfo为null,酒不展示审核按钮
     * 2.如果全部照片审核状态不选 默认全部通过
     * 3.如果人为触发点击通过，不通过，就把人为选择的图片传递接口
    */
    async handleDeviceImgCheck (device) {
      // const haveCheck = device.haveCheck ? device.device_att_proc : device.device_att_proc.filter(f => !!f.filePath)
      // const _length = haveCheck.length
      const _length = device.device_att_proc.length
      const tempArr0 = []// 初始状态
      const tempArr1 = []// 人为选择通过
      const tempArr2 = []// 人为选择不通过

      device.device_att_proc.forEach(f => {
        if (f.deviceimgRadio === 0) {
          tempArr0.push(f)
        } else if (f.deviceimgRadio === 1) {
          tempArr1.push(f)
        } else if (f.deviceimgRadio === 2) {
          tempArr2.push(f)
        }
      })

      let agree_again = ''// 终审通过集合
      let dis_agree_again = ''// 终审不通过集合
      if (tempArr0.length === _length) { // 如果全部照片审核状态不选 默认全部通过
        agree_again = tempArr0.map(m => m.image_id).join(',')
      } else { // 如果人为触发点击通过，不通过，就把人为选择的图片传递接口
        agree_again = tempArr1.map(m => m.image_id).join(',')
        dis_agree_again = tempArr2.map(m => m.image_id).join(',')
      }
      const params = {
        project_code: this.form.projectCode,
        point_code: this.currentPoint.pointCode,
    ...
    // 工序步骤照片审核
    async changeImgAudit (img) {
      const check = img.num
      const agree_again = check ? img.image_id + '' : ''// 终审通过集合
      const dis_agree_again = check ? '' : img.image_id + ''// 终审不通过集合
      const params = {
        project_code: this.form.projectCode,
        point_code: this.currentPoint.pointCode,
        audit_level: '1',
