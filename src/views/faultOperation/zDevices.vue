<template>
  <div class="faultOperationFault">
    ...
    >
      <el-form size="small" inline style="margin-top: 14px">
        ...
        <el-form-item style="margin-bottom: 0">
          <el-button type="primary" size="mini" @click="submitOper"
            >确定</el-button
          >
        </el-form-item>
      </el-form>
    ...
    >
      <record-info :info="rowRecord" :id="rowRecord.id" />
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    async getList() {
      ...
      const params = {
        ...
        oper_status:
          this.form.oper_status === null ? -1 : this.form.oper_status,

        page_no: this.page.page_no,
        page_size: this.page.page_size,
      };
      const { data, code } = await this.$pub.post(
        "/point/order/device/list",
        params
      );
    ...
    getTableHeight() {
      this.$nextTick(() => {
        const el_wrap = document.getElementsByClassName(
          "faultOperationFault"
        )[0];
        if (el_wrap) {
    ...
    // 档案
    handleRecord(row) {
      this.rowRecord = { ...row, oper_status: Number(row.oper_status) };
      this.recordFlag = true;
    },
    closeRecord() {
      this.rowRecord = { oper_status: null };
    ...
    async submitOper() {
      ...
      const { code, message } = await this.$pub.post(
        "/point/order/device/status-change",
        tempobj
      );
      if (code === 200) {
