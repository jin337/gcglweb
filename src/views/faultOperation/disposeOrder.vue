  ...
  <div v-loading="loading" class="disposeOrderBox">
    <el-form :model="form" :rules="rules" ref="formRef" size="small">
      ...
      <div class="topwrap">
        ...
        <div class="btns" v-if="isOpear">
          <el-button size="mini" type="primary" @click="handleSubmit('temp')"
            >暂存</el-button
          >
      ...
      <el-card class="box-card">
        ...
        >
          <vxe-column type="checkbox" min-width="40px" align="center" />
          <vxe-column
            title="项目"
            field="project_name"
            align="center"
          ...
          >
            <template v-slot="{ row, rowIndex }">
              <div class="curp blue" @click="openPoint(row, rowIndex)">
                {{ row.point_code }}
              </div>
            </template>
          </vxe-column>
          ...
          >
            <template v-slot="{ row, rowIndex }">
              ...
              <span class="curp" @click="openFaultInfo(row, rowIndex)">{{
                row.handle_count
              }}</span>
            </template>
          </vxe-column>
          ...
          >
            <template v-slot="{ row, rowIndex }">
              <el-form-item
                class="handle_remark"
                :prop="`tableData.${rowIndex}.handle_remark`"
                v-if="row.checked && row.status === 0"
          ...
          >
            <template v-slot="{ row }">
              <div style="cursor: pointer" @click="handlePreview(row)">
                {{ row.photo_list.length > 0 ? row.photo_list.length : 0 }}
              </div>
            </template>
          </vxe-column>
          <vxe-column
          ...
          >
            <template v-slot="{ row }">
              <span>{{
                row.handle_time ? parseTime(row.handle_time) : ""
              }}</span>
            </template>
          </vxe-column>
        </vxe-table>
            ...
            <div class="hesuanbox">
              ...
              <div class="flexwidth">
                ...
                <div class="hesuanwrap" v-if="false">
                  ...
                  <div
                    class="vux-flexbox"
                    v-if="form.selectedWorkers.length > 0"
                  >
                    <span class="label">人力数量</span>
                    <div class="xunhuanbox">
                      <el-form-item
                        v-for="worker in form.selectedWorkers"
                        :key="worker"
                        :prop="`workerQuantities[${worker}]`"
                      ...
                      >
                        <el-tag style="margin-right: 4px">
                          ...
                          <el-input-number
                            :disabled="!isOpear"
                            v-model="form.workerQuantities[worker]"
                            placeholder="请输入数量"
                            style="width: 86px"
                            size="mini"
                            :min="1"
                ...
                <div class="hesuanwrap" v-if="false">
                  ...
                  <div class="vux-flexbox" v-if="form.selectedCars.length > 0">
                    ...
                    <div class="xunhuanbox">
                      <el-form-item
                        v-for="car in form.selectedCars"
                        :key="car"
                        :prop="`carQuantities[${car}]`"
                        :rules="carQuantityRules"
                      >
                        <el-tag style="margin-right: 4px">
                          <span>{{ getCarName(car) }}：</span>
                          <el-input-number
                            v-model="form.carQuantities[car]"
                            placeholder="请输入数量"
                            style="width: 86px"
                            :disabled="!isOpear"
                            size="mini"
                ...
                <div class="hesuanwrap vux-flexbox" v-if="false">
                  ...
                  <el-form-item prop="carpersonPrice">
                    import { parseTime } from '@/utils/tool'; import moment from
                    'moment'; import mixin from './mixin.js'; readonly
                    :disabled="!isOpear" />
            ...
            <div>
              ...
              <p class="rowcls">
                ...
                <el-form-item prop="movePoint">
                  <el-input
                    v-model="form.movePoint"
                    @blur="handlefinishBlur"
                    :disabled="!isOpear"
    ...
    <el-dialog append-to-body title="修复数量" :visible.sync="faultModel">
      <div class="faultbtns" v-if="isOpear && deviceList.length">
        <el-button size="mini" type="primary" @click="saveFault"
          >确定</el-button
        >
      </div>
...
export default {
  ...
  computed: {
    ...
    carpersonPrice() {
      const workertotalAmount = this.form.selectedWorkers.reduce(
        (total, worker) => {
          const workerInfo = this.workerList.find((w) => w.value === worker);
          if (workerInfo) {
            return (
              total +
              (this.form.workerQuantities[worker] || 0) * workerInfo.price
            );
          }
          return total;
        },
  ...
  watch: {
    ...
    "form.tableData": {
      handler(newVal) {
        ...
        // movePoint：移交运营商点位数
        // moveDevice：移交运营相机数
        this.form.finishPoint = tempArr.filter(
          (f) => f.handle_count > 0
        ).length;
        this.form.finishDevice = tempArr.reduce(
          (sum, item) => sum + item.handle_count,
          0
        );
      },
      deep: true, // 深度监听
  ...
  methods: {
    ...
    async getDictData() {
      const { data, code, message } = await this.$pub.post(
        "/sys/dict/list-slt",
        { dict_label: "oper_status" }
      );
      if (code === 200) {
        this.OperatingStatuslist = (data || []).map((m) => {
          return {
    ...
    async getList() {
      ...
      (data.price_list_first || []).forEach((m) => {
        ...
        //   return; // 跳过本次迭代
        // }
        this.form.estimate += `${m.name}(${m.price})，数量：${m.count}${
          type[m.type]
        }，`;
      });
      ...
      this.form.pDevice = this.form.tableData.reduce(
    ...
    async handleSubmit(name) {
      ...
      this.$refs.formRef.validate(async (valid) => {
        ...
        if (valid) {
          ...
          const workPrice = this.assignWorker(
            this.form.workerQuantities,
            this.workerList
          );
          const carPrice = this.assignWorker(
            this.form.carQuantities,
            this.carList
          );
          ...
          const tiao = {
            ...
            name: "调价",
            value: 0,
            price: Number(this.form.numberMoney),
          };
    ...
    saveFault() {
      ...
      const operator = this.deviceList.filter(
        (item) => item.status === 3
      ).length;
      ...
      this.form.movePoint = this.form.tableData.filter(
        (f) => f.move_count > 0
      ).length;
      this.form.moveDevice = this.form.tableData.reduce((total, item) => {
        return (
          total + (item.device_list?.filter((d) => d.status === 3).length || 0)
        );
      }, 0);
