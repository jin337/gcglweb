<template>
  <div class="import">
    ...
    <div style="margin-top:20px;">
      <span>状态:</span>
      <el-select v-model="status" style="width:calc(100% - 45px);" placeholder="请选择">
        <el-option v-for="item in statusList" :key="item.status" :label="item.status_name" :value="item.status">
        </el-option>
      </el-select>
    </div>
    ...
    <div class="el-upload__tip" slot="tip">
      <p>导入须知:</p>
      <p style="color:red;">{{ config.aramMessage }}</p>
      <p>文件格式目前仅支持XLSX</p>
    ...
    <el-button type="primary" style="width: 100%;margin-top:20px;"
      :disabled="!this.projectCode || this.status === null || fileList.length <= 0" :loading="submitloading"
      @click="handleUpload">确认</el-button>
