export default {
  ...
  methods: {
    editPointName(row) {
      ...
      this.$msgbox({
        ...
        message: h(
          ...
          {
            style: 'display: flex',
            class: { 'el-input': true, 'el-input--mini': true },
          },
          [
        ...
        beforeClose: async (action, instance, done) => {
          if (action === 'confirm') {
            ...
            const { code } = await this.$pub.post(
              '/point/manage/name-edit',
              params
            )
            if (code !== 200) {
              instance.confirmButtonLoading = false
    ...
    manyClick(row, type) {
      ...
      } else if (type === 'oper_status') {
        this.click = {
          title: '运维状态',
          list: this.opers.list,
          label: 'oper_status_name',
          value: 'oper_status',
          type: 'oper_status',
        }
        this.permission = ['points:statusEdit']
