...
export default {
  ...
  methods: {
    ...
    add() {
      ...
      this.form.tagvalue = ['4561', '2272', '1717']
      this.addFlag = true
      this.getTagList()
    },
    // 新增 start
        ...
        this.$refs.addForm.validate(async valid => {
          if (valid) {
            ...
            const query_info = {
              ...
              begin_time: time[0] ? time[0] : null, // + ' 00:00:00'
              end_time: time[1] ? time[1] : null, // + ' 23:59:59'
              tags: this.form.tagvalue
            }

            const { data, code, message } = await this.$pub.post('/gen/device-online-report', query_info)
            if (code !== 200) {
              this.$message({
                message: message,
                type: 'info',
