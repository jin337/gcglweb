<template>
  <div class="realtimep">
    ...
    <el-dialog title="物料统计新增" width="550px"  :visible.sync="addFlag" class="realtime_di">
      <el-form ref="addForm" size="small" label-position="right" label-width="78px" :rules="rules" :model="form" v-if="addFlag">
        ...
        <el-form-item label="状态">
              <el-radio-group  v-model="form.audit_status">
                <el-radio v-for="item in status_list" :label="item.dict_value" :key="item.dict_value">{{item.dict_label}}</el-radio>
              </el-radio-group>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      status_list: [
        { dict_label: '初审', dict_value: 1 }
        // , { dict_label: '终审', dict_value: 2 }
      ],
      submitloading: false
    }
  },
        ...
        this.$refs.addForm.validate(async valid => {
          if (valid) {
            ...
            const query_info = {
              ...
              status: this.form.audit_status
            }
            const { data, code, message } = await this.$pub.post('/gen/device_proc_doc', query_info)
            if (code !== 200) {
              this.$message({
                message: message,
                type: 'info',
